...
\begin{figure}
\transparent{0.75}\includegraphics[scale=0.5]{Figures/sol1d/PRbar600.png}
\centering
...
\large{\textbf{POWER FLUX INPUT VARIATION THOUGHTS}}
\normalsize


PF variation figure shows the successful disconnect SXDG achieves between the midplane and the divertor plates. 

A 2.5 x increase in power flux input only causes noticeable \Ttg increase at very low set \neus values - more investigation is required but from these results it seems that at sufficiently high density there is no drawback to ramping up core power, and consequently power flux input to the SOL.

Increasing \neus sees a steady decrease in \neus, but SXD geometry is such that detachment progresses steadily and the divertor plasma does not suddenly pull away from the target in a small \neus window.

\textbf{could fit straight lines to \Tus's to see the effect on slope of changing PFI}
